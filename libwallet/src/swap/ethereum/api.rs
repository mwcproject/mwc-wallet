	...
	E: EthNodeClient + 'a,
{
	...
	fn buyer_refund<K: Keychain>(
		...
		let c = self.eth_node_client.lock();
		let eth_data = swap.secondary_data.unwrap_eth()?;
		c.refund(eth_data.address_from_secret.clone().unwrap())
	}
	...
	fn buyer_deposit(&self, swap: &mut Swap) -> Result<H256, ErrorKind> {
		...
		nc.initiate(refund_time, address_from_secret, participant, value)
	}
	...
	E: EthNodeClient + 'a,
{
	...
	fn is_secondary_tx_fee_changed(&self, _swap: &Swap) -> Result<bool, ErrorKind> {
		//todo
		Ok(false)
	}
