...
pub fn create_command(class: u8, instruction: u8, p1: u8, p2: u8, data: Vec<u8>) -> APDUCommand<Vec<u8>> {
    APDUCommand {
        cla: class,
        ins: instruction,
        p1,
...
pub enum APDUCommands {
    ...
    GetAddress { account: u32, index: u32, address_type: u8 },
    ...
    GetCommitment { account: u32, identifier: [u8; 17], value: u64, switch_type: u8 },
    ...
    VerifyAddress { account: u32, index: u32, address_type: u8 },
    ...
    StartEncryptingSlate { account: u32, index: u32, recipient_address: Vec<u8> },
    ...
    ContinueTransactionGetMessageSignature { 
        message: Vec<u8> 
    },
...
impl APDUCommands {
    ...
    pub fn to_apdu_command(&self) -> APDUCommand<Vec<u8>> {
        match self {
            APDUCommands::GetRootPublicKey { account } => create_command(0xC7, 0x00, 0x00, 0x00, account.to_le_bytes().to_vec()),

            APDUCommands::GetAddress { account, index, address_type } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(&index.to_le_bytes());
                create_command(0xC7, 0x01, *address_type, 0x00, data)
            }

            APDUCommands::GetSeedCookie { account } => create_command(0xC7, 0x02, 0x00, 0x00, account.to_le_bytes().to_vec()),

            APDUCommands::GetCommitment { account, identifier, value, switch_type } => {
                let mut data = account.to_le_bytes().to_vec();
            ...
            APDUCommands::GetBulletproofComponents { account, identifier, value, switch_type, message_type } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(identifier);
                data.extend_from_slice(&value.to_le_bytes());
                data.push(*switch_type);
            ...
            APDUCommands::VerifyRootPublicKey { account } => create_command(0xC7, 0x05, 0x00, 0x00, account.to_le_bytes().to_vec()),
            
            APDUCommands::VerifyAddress { account, index, address_type } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(&index.to_le_bytes());
                create_command(0xC7, 0x06, *address_type, 0x00, data)
            }
            APDUCommands::StartEncryptingSlate { account, index, recipient_address } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(&index.to_le_bytes());
                data.extend_from_slice(recipient_address);
                create_command(0xC7, 0x07, 0x00, 0x00, data)
            }
            APDUCommands::ContinueEncryptingSlate { data } => create_command(0xC7, 0x08, 0x00, 0x00, data.clone()),
            APDUCommands::FinishEncryptingSlate => create_command(0xC7, 0x09, 0x00, 0x00, vec![]),
            APDUCommands::StartDecryptingSlate { account, index, nonce, sender_address, salt } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(&index.to_le_bytes());
            ...
            APDUCommands::ContinueDecryptingSlate { data } => create_command(0xC7, 0x0B, 0x00, 0x00, data.clone()),
            APDUCommands::FinishDecryptingSlate { tag } => create_command(0xC7, 0x0C, 0x00, 0x00, tag.to_vec()),
            APDUCommands::StartTransaction { account, index, output, input, fee, secret_nonce_index, address } => {
                let mut data = account.to_le_bytes().to_vec();
                data.extend_from_slice(&index.to_le_bytes());
            ...
            APDUCommands::ContinueTransactionIncludeOutput { identifier, value, switch_type } => {
                let mut data = identifier.to_vec();
                data.extend_from_slice(&value.to_le_bytes());
                data.push(*switch_type);
                create_command(0xC7, 0x0E, 0x00, 0x00, data)
            }
            APDUCommands::ContinueTransactionIncludeInput { identifier, value, switch_type } => {
                let mut data = identifier.to_vec();
                data.extend_from_slice(&value.to_le_bytes());
                data.push(*switch_type);
                create_command(0xC7, 0x0F, 0x00, 0x00, data)
            }
            APDUCommands::ContinueTransactionApplyOffset { offset } => create_command(0xC7, 0x10, 0x00, 0x00, offset.to_vec()),
            APDUCommands::ContinueTransactionGetPublicKey => create_command(0xC7, 0x11, 0x00, 0x00, vec![]),
            APDUCommands::ContinueTransactionGetPublicNonce => create_command(0xC7, 0x12, 0x00, 0x00, vec![]),
            APDUCommands::ContinueTransactionGetMessageSignature { message } => create_command(0xC7, 0x13, 0x00, 0x00, message.clone()),
