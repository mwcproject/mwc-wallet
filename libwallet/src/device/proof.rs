...
impl ProofBuilder {
    pub fn new(root_public_key: &[u8]) -> Self {
        ...
        let rewind_hash = <[u8; 32]>::try_from(hash_result.as_slice()).expect("Hash should be 32 bytes");
    ...
    pub async fn get_rewind_nonce(&self, commitment: &[u8]) -> Result<[u8; 32], &'static str> {
        ...
        let hash_result = hasher.finalize();
        let rewind_nonce = <[u8; 32]>::try_from(hash_result.as_slice()).expect("Hash should be 32 bytes");
    ...
    pub fn decode_message(message: &[u8]) -> Result<(Identifier, u8), &'static str> {
        ...
        let identifier = Identifier(&message[MESSAGE_IDENTIFIER_INDEX..MESSAGE_IDENTIFIER_INDEX + IDENTIFIER_LENGTH]);

        Ok((identifier, switch_type))
    }

    pub fn encode_message(identifier: &Identifier, switch_type: u8) -> Result<Vec<u8>, &'static str> {
        if switch_type > 0xff {
        ...
        message[MESSAGE_IDENTIFIER_INDEX..MESSAGE_IDENTIFIER_INDEX + IDENTIFIER_LENGTH].copy_from_slice(&identifier.to_bytes());
