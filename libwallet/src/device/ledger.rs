...
use secp256k1zkp::{PublicKey, SecretKey, Secp256k1};
...
pub fn send_apdu_command(
    ...
    let transport = transport.exchange(&apdu_command).map_err(|e| ParseError::ApduError(e.to_string()));
    match transport {
        Ok(r) => {
            match r.retcode() {
                ...
                code => {
                    let status = match code {
                        ...
                        _ => return Err(ParseError::ApduError(format!("Unknown status code: 0x{:X}", code))),
                    };
...
/// Verify all aspects of a completed payment proof on the current slate
pub fn get_rewind_nonce(secp: &Secp256k1, commit: Commitment, hash: Vec<u8>) -> Result<SecretKey, Error> {
...
pub fn get_root_public_key(transport: &TransportNativeHID, account: u32) -> Result<Vec<u8>, ParseError> {
    let response = send_apdu_command(&transport, APDUCommands::GetRootPublicKey { account: account}).unwrap();
    let data = response;
    {
        if data.len() != 33 {
...
pub fn verify_root_public_key(transport: &TransportNativeHID, account: u32) -> Result<(), ParseError> {
...
pub fn get_bulletproof_components(
    ...
    let tau_x = SecretKey::from_slice(&secp, &subarray(&get_bulletproof_components_response, 0, Some(TAU_X_LENGTH)))
        .map_err(|_| ParseError::SecretKeyParseError)?;

    let t_one = PublicKey::from_slice(&secp,&subarray(
        &get_bulletproof_components_response,
        TAU_X_LENGTH,
        Some(TAU_X_LENGTH + SECP256K1_PUBLIC_KEY_LENGTH),
    )).map_err(|_| ParseError::PublicKeyParseError)?;

    let t_two = PublicKey::from_slice(&secp,&subarray(
        &get_bulletproof_components_response,
