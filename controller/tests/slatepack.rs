...
fn output_slatepack(
	...
	.map_err(|e| {
		mwc_wallet_libwallet::Error::GenericError(format!("Unable to store the slate, {}", e))
	})?;
	Ok(())
}
...
fn slate_from_packed(
	...
	match PathToSlateGetter::build_form_path(0, file.into())
		...
		.map_err(|e| {
			mwc_wallet_libwallet::Error::GenericError(format!("Unable to read the slate, {}", e))
		})? {
		// Plain slate, V2 or V3
		SlateGetData::PlainSlate(_) => {
			return Err(mwc_wallet_libwallet::Error::GenericError(
...
/// self send impl
fn slatepack_exchange_test_impl(test_dir: &str) -> Result<(), mwc_wallet_libwallet::Error> {
	...
	let (_address1, recipients_1, secret_1, sender_1) = {
		...
		mwc_wallet_controller::controller::owner_single_use(
			...
			None,
			|api, m| {
				let mut w_lock = api.wallet_inst.lock().unwrap_or_else(|e| e.into_inner());
	...
	mwc_wallet_controller::controller::foreign_single_use(
		wallet2.clone(),
		mask2_i.clone(),
		|api| {
