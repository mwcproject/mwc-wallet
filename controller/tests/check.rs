...
use mwc_wallet_impls::{PathToSlatePutter, SlatePutter};
...
fn scan_impl(test_dir: &str) {
	...
	let reward = consensus::MWC_FIRST_GROUP_REWARD;
	let cm = global::coinbase_maturity(0) as u64; // assume all testing precedes soft fork height
	...
	mwc_wallet_controller::controller::owner_single_use(
		...
		|api, m| {
			...
			let (c, _) = mwc_wallet_libwallet::TxLogEntry::sum_confirmed(&txs);
	...
	.unwrap();
	let w1_outputs: Vec<mwc_wallet_libwallet::OutputData> =
		w1_outputs_commits.into_iter().map(|m| m.output).collect();
	{
		wallet_inst_test!(wallet1, w);
		{
			...
			accidental_spent.status = mwc_wallet_libwallet::OutputStatus::Spent;
...
fn two_wallets_one_seed_impl(test_dir: &str) {
	...
	// few values to keep things shorter
	let _reward = consensus::MWC_FIRST_GROUP_REWARD;
	let cm = global::coinbase_maturity(0) as usize; // assume all testing precedes soft fork height
...
// is all here
fn output_scanning_impl(test_dir: &str) -> Result<(), mwc_wallet_controller::Error> {
	global::set_local_chain_type(global::ChainTypes::AutomatedTesting);
	let tx_pool: Arc<Mutex<Vec<Transaction>>> = Arc::new(Mutex::new(Vec::new()));
	let mut wallet_proxy = create_wallet_proxy(test_dir.into(), tx_pool.clone());
	let chain = wallet_proxy.chain.clone();
