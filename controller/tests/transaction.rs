...
//! tests for transactions building within mwc_wallet_util::mwc_core::libtx
use mwc_wallet_util::mwc_crates::log::{debug, error};

use mwc_wallet_impls::test_framework::{self, LocalWalletClient};
use mwc_wallet_libwallet::{InitTxArgs, OutputStatus, Slate};
...
fn basic_transaction_api(test_dir: &str) -> Result<(), mwc_wallet_controller::Error> {
	...
	let amount = mwc_core::consensus::MWC_FIRST_GROUP_REWARD;
	...
	mwc_wallet_controller::controller::owner_single_use(
		...
		|sender_api, m| {
			...
			assert_eq!(est.amount, 10 * mwc_core::consensus::MWC_FIRST_GROUP_REWARD);
			...
			let est = sender_api.init_send_tx(m, None, &init_args, 1)?;
			assert_eq!(est.amount, mwc_core::consensus::MWC_FIRST_GROUP_REWARD * 3);
...
/// posted to a chain
fn tx_rollback(test_dir: &str) -> Result<(), mwc_wallet_controller::Error> {
	global::set_local_chain_type(global::ChainTypes::AutomatedTesting);
	// Create a new proxy to simulate server and wallet responses
	let tx_pool: Arc<Mutex<Vec<Transaction>>> = Arc::new(Mutex::new(Vec::new()));
	let mut wallet_proxy = create_wallet_proxy(test_dir.into(), tx_pool.clone());
	...
	let amount = mwc_core::consensus::MWC_FIRST_GROUP_REWARD / 2;
	let mut slate = Slate::blank(1, false);
	mwc_wallet_controller::controller::owner_single_use(
		Some(wallet1.clone()),
		mask1,
